...
def test_get_experiments():
    response = requests.get(f'http://{HOSTNAME}:{PORT}/experiments/experiments_for_proposal/12345')
    a = response.json()
...
def test_begin_experiment():
    response = requests.put(f'http://{HOSTNAME}:{PORT}/experiments/experiment/13579/1')

def test_end_experiment():
    response = requests.get(f'http://{HOSTNAME}:{PORT}/experiments/experiment')
    a = response.json()
    assert a['proposal'] == '13579'
    assert a['experiment'] == '1'

def test_begin_experiment_again():
    response = requests.put(f'http://{HOSTNAME}:{PORT}/experiments/experiment/12345/1')
