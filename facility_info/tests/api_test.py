...
def test_user_visits():
    response = requests.get(f'http://{HOSTNAME}:{PORT}/expt_session/user_visits/jdoe')
    a = response.json()
    assert a['visits'][0] == '12345'

def test_visit_times():
    response = requests.get(f'http://{HOSTNAME}:{PORT}/expt_session/times/12345')
    a = response.json()
...
def test_begin_session():
    response = requests.put(f'http://{HOSTNAME}:{PORT}/expt_session/visit/13579')

def test_end_session():
    response = requests.get(f'http://{HOSTNAME}:{PORT}/expt_session/visit')
    a = response.json()
    assert a['visit'] == '13579'

def test_begin_session_again():
    response = requests.put(f'http://{HOSTNAME}:{PORT}/expt_session/visit/12345')

def test_end_session():
    response = requests.get(f'http://{HOSTNAME}:{PORT}/expt_session/visit')
    a = response.json()
    assert a['visit'] == '12345'
